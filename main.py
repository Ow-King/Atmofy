...
def main():
    ...
    temp, weekday, clock, sky = weather.getWeather()
    ...
    user.store_current_song("songsPayed.txt", tid, 0)
    ...
    while True:
        artist, track, tid = user.get_current_song()
        if (user.check_song_change("songsPayed.txt", tid)):
            user.store_current_song("songsPayed.txt", tid, timePlayed)
